...
export default function ScreenTextEngine(
  ...
  function scroll(
    ...
    // Calculate the maximum possible scroll based on content height
    // charNextLoc.y holds the current vertical cursor position (as a positive offset from top)
    const contentHeight = charNextLoc.y;
    const viewportHeight = 1.0;
    const maxScrollLimit = Math.max(0, contentHeight - viewportHeight);
    ...
    if (rootGroup.position.y > maxScrollLimit) rootGroup.position.y = maxScrollLimit;
  }
  ...
  function scrollToEnd() {
    ...
    if (rootGroup.position.y !== limit) rootGroup.position.y = limit;
  }
